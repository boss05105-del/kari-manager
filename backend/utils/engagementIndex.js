...
function calcEngagementIndex(storeId) {
  const db = getDb();
  const endDate = new Date();
  // Always use current calendar month
  ...
  const facts = db.prepare(`
    ...
    WHERE store_id = ? AND fact_date >= ? AND fact_date <= ?
    ORDER BY fact_date
  `).all(storeId, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0]);

  const plansByDate = {};
  const factsByDate = {};
  plans.forEach(p => { plansByDate[p.plan_date] = p; });
  ...
  let consecutiveStreak = 0, maxStreak = 0, currentStreak = 0;

  const cursor = new Date(startDate);
  while (cursor <= endDate) {
    const dayOfWeek = cursor.getDay();
    if (dayOfWeek !== 0) { // all days except Sunday
      workingDays++;
      ...
      if (plan && fact) {
        ...
        if (completion !== null) {
          kpiScoreTotal += Math.min(completion, 1);
          kpiScoreCount++;
        }
        const commentLen = (plan.comment?.length || 0) +
          (fact.what_helped?.length || 0) +
          (fact.obstacles?.length || 0) +
  ...
  if (workingDays === 0) return { index: 0, breakdown: {} };

  // Plan timeliness (0-25)
  const planScore = ((plansOnTime * 25 + plansLate * 12) / workingDays);

  // Fact timeliness (0-25)
  ...
  return {
    ...
    stats: {
      workingDays,
      plansOnTime, plansLate, plansMissed,
